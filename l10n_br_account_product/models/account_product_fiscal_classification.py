...
class L10nBrTaxDefinitionTemplateModel(L10nBrTaxDefinitionTemplate):
    ...
    cfop_estado_id = fields.Many2one(
        'l10n_br_account_product.cfop', 'CFOP Estado')
    cfop_outros_id = fields.Many2one(
        'l10n_br_account_product.cfop', 'CFOP Outros')
    cfop_exterior_id = fields.Many2one(
        'l10n_br_account_product.cfop', 'CFOP Exterior')
    cfop_entrada_id = fields.Many2one(
        'l10n_br_account_product.cfop', 'CFOP Entrada')
    ...
    def button_update_products(self, cr, uid, ids, context=None):
        ...
        for fiscal_classification in self.browse(cr, uid, ids):
            ...
            for product in obj_product.browse(cr, uid, product_ids, context):
                ...
                vals = {
                    'taxes_id': [
                        (6, 0, list(set(to_keep_sale_tax_ids + [x.id for x in fiscal_classification.sale_tax_ids])))],
                    'supplier_taxes_id': [(6, 0, list(
                        set(to_keep_purchase_tax_ids + [x.id for x in fiscal_classification.purchase_tax_ids])))],
                }
